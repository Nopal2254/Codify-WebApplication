...
@section('title', __('Watch Later') . ' | Codify')
...
<div class="container" style="padding-top: 100px; padding-bottom: 60px;">
    ...
    <div class="page-header" style="margin-bottom: 40px;">
        <h1 class="page-title" style="font-size: 32px; background: linear-gradient(135deg, #fff 0%, var(--brand) 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">{{ __('Watch Later') }}</h1>
        <p style="color: var(--text-muted); margin-top: 8px;">{{ __('Your saved courses to watch later.') }}</p>
    </div>
    ...
    @if($watchLaterItems->count() > 0)
        <div class="glass-card" style="padding: 28px; background: rgba(255, 255, 255, 0.02); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: var(--radius-lg);">
            <div class="row g-4">
                @foreach($watchLaterItems as $item)
                    @if($item->course)
    ...
    @else
        <div class="glass-card" style="padding: 60px 40px; text-align: center; background: rgba(255, 255, 255, 0.02); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: var(--radius-lg);">
            ...
            <h3 style="font-size: 20px; font-weight: 800; color: #fff; margin-bottom: 8px;">{{ __('Your watch later list is empty') }}</h3>
            <p style="color: var(--text-muted); font-size: 14.5px; max-width: 420px; margin: 0 auto 24px;">{{ __('Save courses to watch later from the course page to watch them when you have time.') }}</p>
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.watch-later-toggle-form').forEach(form => {
